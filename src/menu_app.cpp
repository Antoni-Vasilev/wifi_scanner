...
void MenuApp::moveUp() {
  ...
  if (selectedIndex > 0) {
    ...
    selectedIndex = currentMenu->itemCount - 1;
  }
...
void MenuApp::render() {
  ...
  if (endIndex > currentMenu->itemCount) {
    endIndex = currentMenu->itemCount;
  }
...
bool MenuApp::needsRedraw() const {
  return redraw;
}

void MenuApp::clearRedrawFlag() {
  redraw = false;
}